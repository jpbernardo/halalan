...
<div class="content_right">
	<p class="align_right"><?php
$uadmin = $this->session->userdata('admin');
$u = $uadmin['electionid'];
if($u == 1) {		
 echo anchor('admin/elections/add', e('admin_elections_add')); 
}
?></p>
</div>
<div class="clear"></div>
	...
	<tr class="<?php echo ($i % 2 == 0) ? 'odd' : 'even'  ?>">
		...
		<td align="center">
			...
			<?php 
		              $uadmin = $this->session->userdata('admin');
			      $u = $uadmin['electionid'];
			      if($u == 1 && !$election['status']) { ?>
        		    | (<?php echo anchor('admin/elections/options/download/' . $election['id'], 'download'); ?>)
           		 <?php } ?>
			<?php else: ?>
